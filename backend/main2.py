...
from database.teams import get_teams, get_teams_by_ids  # Updated import
...
from database.test_data import get_test_constraints
...
def generate_schedule(schedule_name: str = "Generated Schedule", constraints_list: Optional[List[Any]] = None) -> int:
    """
    Generates a schedule using hardcoded values:
    - facility_id = 4
    - club_id = 5
    Teams are automatically fetched from the database using club_id
    Returns the schedule_id if successful.
    """
    profiler = cProfile.Profile()
    profiler.enable()

    facility_id = 4  # Hardcoded facility_id
    club_id = 5      # Hardcoded club_id
    
    # Fetch active teams for the club
    ...
    if constraints_list:
        ...
        # Use test constraints instead of fetching from database
        constraints_list = get_test_constraints()
        for constraint in constraints_list:
...
if __name__ == "__main__":
    try:
        ...
        print(f"Successfully generated schedule with ID: {schedule_id}")
