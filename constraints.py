...
def add_no_double_booking_constraints(model, teams, interval_vars, assigned_fields, subfield_areas, global_time_slots):
    ...
    """
    subfield_intervals = defaultdict(list)
            ...
            for session_idx, session in enumerate(sessions):
                for part_idx, (interval, assigned_combo_var) in enumerate(zip(session['intervals'], session['assigned_combos'])):
                    ...
                    for combo, combo_index in combo_indices.items():
                        ...
                        uses_combo = model.NewBoolVar(f'uses_{team_name}_{idx_constraint}_{session_idx}_{part_idx}_{combo_index}')
                        ...
                        for sf in subfields:
                            sf_interval = model.NewOptionalIntervalVar(
                                ...
                                f'sf_interval_{team_name}_{idx_constraint}_{session_idx}_{part_idx}_{sf}'
                            )
