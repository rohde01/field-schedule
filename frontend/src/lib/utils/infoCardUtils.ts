...
export function createInfoCardForm(entry: ScheduleEntry): Writable<Record<string, any>> {
  return writable({
...
function createInfoCardStore() {
  ...
  return {
    subscribe,
    openEventInfoCard: (e: MouseEvent, entry: ScheduleEntry, containerElement: HTMLElement) => {
      e.stopPropagation();
      const containerRect = containerElement.getBoundingClientRect();
      const targetRect = (e.currentTarget as HTMLElement).getBoundingClientRect();
      
      const editingEventPosition = calculateInfoCardPosition(containerRect, targetRect);
      const infoCardForm = createInfoCardForm(entry);
